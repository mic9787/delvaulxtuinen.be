   ...
   var hasParentClass = function(element, classname) {
	...
	function init() {
        ...
        buttons = Array.prototype.slice.call( document.querySelectorAll( 'button' ) )
			// buttons = document.getElementById( 'js-showOffer' ),
			// event type (if mobile use touch events)
			eventtype = mobilecheck() ? 'touchstart' : 'click',
			resetMenu = function() {
				document.getElementById( 's-container' ).classList.remove( 'sidebar-open' );
			},
			...
			document.getElementById( 's-btn' ).addEventListener( eventtype, function( ev ) {
				ev.stopPropagation();
				ev.preventDefault();
				container.className = 's-container'; // clear
				// document.getElementById( 's-container' ).classList.add(effect);
				setTimeout( function() {
					document.getElementById( 's-container' ).classList.add('sidebar-open');
				}, 25 );
				document.addEventListener( eventtype, bodyClickFn );
			});
      document.getElementById( 'jsclose' ).addEventListener( eventtype, function( ev ) {
        ev.stopPropagation();
				ev.preventDefault();
        resetMenu();
      });
